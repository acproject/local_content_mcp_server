...
// ContentManager实现
ContentManager::ContentManager(std::shared_ptr<Database> db)
    : db_(std::move(db)) {}
...
nlohmann::json ContentManager::update_content(int64_t id,
...
nlohmann::json ContentManager::search_content(const std::string &query,
  ...
  try {
    ...
    result.items = items;
    result.total_count =
        static_cast<int>(items.size()); // 简化实现，实际应该查询总数
    result.page = page;
...
nlohmann::json ContentManager::get_content_by_tag(const std::string &tag,
                                                  int page, int page_size) {
  try {
    if (tag.empty()) {
...
// 辅助方法实现
nlohmann::json ContentManager::create_error_response(const std::string &message,
                                                     int code) {
...
nlohmann::json
ContentManager::create_success_response(const nlohmann::json &data) {
  nlohmann::json response;
...
bool ContentManager::validate_content_item(const nlohmann::json &item,
                                           std::string &error_msg) {
  if (!item.is_object()) {
  ...
  if (item.contains("content_type")) {
    ...
    const std::vector<std::string> valid_types = {
        "text", "markdown", "html", "code", "json", "xml", "yaml", "document"};

    if (std::find(valid_types.begin(), valid_types.end(), content_type) ==
        valid_types.end()) {
...
ContentManager::parse_tags(const std::string &tags_str) {
