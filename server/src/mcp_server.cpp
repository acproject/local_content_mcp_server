...
void MCPServer::initialize_tools() {
  ...
  create_tool.input_schema = {
      ...
      {"properties",
       {{"title", {{"type", "string"}, {"description", "Content title"}}},
        ...
        {"content_type",
         {{"type", "string"},
          {"description", "Content type (text, markdown, code, etc.). "
                          "'document' is alias for 'text'."},
          {"default", "text"}}},
        {"tags", {{"type", "string"}, {"description", "Comma-separated tags"}}},
        {"metadata",
         {{"type", "object"}, {"description", "Additional metadata"}}}}},
      {"required", {"title", "content"}}};
  tools_[create_tool.name] = create_tool;
  tool_handlers_[create_tool.name] = [this](const nlohmann::json &args) {
    return tool_create_content(args);
  };
  ...
  tool_handlers_[get_tool.name] = [this](const nlohmann::json &args) {
    return tool_get_content(args);
  };
  ...
  tool_handlers_[update_tool.name] = [this](const nlohmann::json &args) {
    return tool_update_content(args);
  };
  ...
  tool_handlers_[delete_tool.name] = [this](const nlohmann::json &args) {
    return tool_delete_content(args);
  };
  ...
  tool_handlers_[search_tool.name] = [this](const nlohmann::json &args) {
    return tool_search_content(args);
  };
  ...
  tool_handlers_[list_tool.name] = [this](const nlohmann::json &args) {
    return tool_list_content(args);
  };
  ...
  tags_tool.input_schema = {{"type", "object"}, {"properties", {}}};
  tools_[tags_tool.name] = tags_tool;
  tool_handlers_[tags_tool.name] = [this](const nlohmann::json &args) {
    return tool_get_tags(args);
  };
  ...
  stats_tool.input_schema = {{"type", "object"}, {"properties", {}}};
...
nlohmann::json MCPServer::handle_initialize(const nlohmann::json &params) {
  ...
  response["capabilities"] = {{"tools", nlohmann::json::object()},
                              {"resources", nlohmann::json::object()}};
  response["serverInfo"] = {{"name", "Local Content MCP Server"},
                            {"version", "1.0.0"}};
...
nlohmann::json MCPServer::handle_call_tool(const std::string &tool_name,
  ...
  try {
    ...
    nlohmann::json response;
    response["content"] = {{{"type", "text"}, {"text", result.dump(2)}}};

    return response;

  } catch (const std::exception &e) {
    spdlog::error("Error calling tool {}: {}", tool_name, e.what());
    return create_error_response(-2, "Tool execution error: " +
                                         std::string(e.what()));
  }
}
...
nlohmann::json MCPServer::get_server_info() const {
  ...
  info["version"] = "1.0.0";
  info["description"] =
      "A local content management server implementing the MCP protocol";
...
nlohmann::json MCPServer::tool_get_content(const nlohmann::json &args) {
  if (!args.contains("id") || !args["id"].is_number_integer()) {
    return create_error_response(
        -1, "ID parameter is required and must be an integer");
  }
...
nlohmann::json MCPServer::tool_search_content(const nlohmann::json &args) {
  if (!args.contains("query") || !args["query"].is_string()) {
    return create_error_response(
        -1, "Query parameter is required and must be a string");
  }
...
// 辅助方法实现
nlohmann::json MCPServer::create_error_response(int code,
...
nlohmann::json
MCPServer::create_success_response(const nlohmann::json &result) {
  nlohmann::json response;
  response["result"] = result;
  return response;
}

bool MCPServer::validate_request(const nlohmann::json &request,
                                 std::string &error_msg) {
